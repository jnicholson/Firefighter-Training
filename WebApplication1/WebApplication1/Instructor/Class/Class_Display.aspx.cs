...
namespace WebApplication1.Instructor.Class
{
    public partial class Class_Display : System.Web.UI.Page
    {
        ...
        public List<HalonModels.Class> GetClasses()
        {
            ...
            int ffId = ffquery.FirstOrDefault().Firefighter_ID;
            IQueryable<HalonModels.Class> query = _db.Classes;
            query = query.Where(c => c.Firefighter_ID == ffquery.FirstOrDefault().Firefighter_ID);
            ...
            {
                dateList.Add(DateTime.Parse(classList[i].Class_Date));
                startList.Add(DateTime.Parse(classList[i].Class_Date));
            }
        ...
        protected void EditClassButton_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < ClassInfo.Rows.Count; i++)
            {
                ...
                if (checkbox.Checked)
                {
                    ...
                    if (myClass != null)
                    {
                        string pageUrl = "/Instructor/Class/Class_Add.aspx";
